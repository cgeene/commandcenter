...
`CC_MAIN_MODEL`; override with `CC_MAIN_MODEL=opus` / `--model opus`). One live
main agent at a time. Dashboard/CLI/MCP task creation uses an explicit workspace
and is delivered here before any worker is spawned.
