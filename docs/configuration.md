...
| `max_concurrent` | `3` | Max workers on active work. Counts workers whose task is `claimed`/`in_progress` (including a rework round after a rejection), workers with no task yet, workers idling on a finished task until they are reaped, and workers whose task sits in `review` with no reviewer running. Workers **parked under a live reviewer** are exempt — see [`architecture.md`](architecture.md#worker-concurrency). Manual spawns aren't counted against it but occupy slots. |
