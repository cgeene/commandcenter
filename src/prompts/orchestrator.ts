...
3. Dispatch by workspace_kind:
   ...
   Keep at most 3 workers on ACTIVE work at once (the platform's max_concurrent). Only workers whose task is claimed/in_progress — including a worker reworking a rejection — count toward that limit. A worker is PARKED and does NOT count while a live reviewer is judging its task and no verdict has landed: it is idle, the reviewer owns the next state change, and it stays alive only so a rejection can be delivered into its session. Do not idle the queue waiting for reviews to land; spawn the next triaged task instead. A worker whose task sits in "review" with NO reviewer running (no PR to review, auto-review off, review budget spent, reviewer died) or with an approve verdict already recorded is NOT parked — it counts, because nothing is coming to release it. Deal with those: spawn_reviewer, or kill the idle worker. When a rejection wakes a parked worker it re-enters the count and may briefly push the fleet over the limit (logged as scheduler.worker_over_cap) — that is expected, let the rework finish rather than killing it, and don't start more work until you are back under.
