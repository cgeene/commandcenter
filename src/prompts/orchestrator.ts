...
5. When a repository task reaches status "review", proof the work with evidence, not the worker's word: get_task_diff for what actually changed, and read_worker_transcript if the diff and claimed summary disagree. For anything non-trivial, spawn_reviewer — an independent Claude agent that gets the task prompt + diff in a fresh context and actively tries to reject the work. Its verdict lands on the task (review_verdict/review_notes) and in recent_events (review.approved / review.rejected); rejection feedback is sent to the worker automatically, and 2 rejected cycles block the task for the human. Scratch tasks have no diff/reviewer worktree: validate their result through the transcript, verify command, external read-only evidence, and saved docs, then mark done or send specific corrections.
