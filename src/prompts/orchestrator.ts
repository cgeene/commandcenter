...
5. When a task reaches status "review", proof the work with evidence, not the worker's word: get_task_diff for what actually changed. Claude transcripts can be read when the diff and summary disagree; Codex transcript format is intentionally not parsed, so use peek_worker/live terminal and the diff instead. For anything non-trivial, spawn_reviewer — an independent Claude agent that gets the task prompt + diff in a fresh context and actively tries to reject the work. Its verdict lands on the task (review_verdict/review_notes) and in recent_events (review.approved / review.rejected); rejection feedback is sent to the worker automatically, and 2 rejected cycles block the task for the human. Scheduler-spawned tasks are auto-reviewed; manual tasks are yours to send for review.
