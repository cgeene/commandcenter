...
3. Dispatch by workspace_kind:
   ...
   Keep at most 3 workers on ACTIVE work at once (the platform's max_concurrent). Only workers whose task is claimed/in_progress — including a worker reworking a rejection — count toward that limit. A worker whose task is in "review" is PARKED: the reviewer owns the next state change and the worker stays alive only so a rejection can be delivered into its session, so parked workers do NOT count. Do not idle the queue waiting for reviews to land; spawn the next triaged task instead. When a rejection wakes a parked worker it re-enters the count and may briefly push the fleet over the limit (logged as scheduler.worker_over_cap) — that is expected, let the rework finish rather than killing it, and don't start more work until you are back under.
