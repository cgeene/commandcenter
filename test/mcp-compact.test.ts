...
describe("main-role tools", () => {
  ...
  it("get_task acks the triage ping on a full read only", async () => {
    // Reading the whole record IS the triage action, so it doubles as the ack
    // that stops the platform re-delivering the task.
    await callTool("get_task", { id: 42, verbose: true });
    expect(fetchCalls.at(-1)?.path).toBe("/api/tasks/42?triage_ack=1");
